...
$img_cat = array('med', 'small', 'thumb', 'large', 'xlarge');
...
/* work out the required image size and set it as $resolution*/
$arr_requested_uri = explode('/', $requested_uri);
$req_img_cat = $arr_requested_uri[count($arr_requested_uri) - 2];
$screen_resolution = getResolution($resolutions);
for ($i=count($resolutions)-1; $i>0; $i--) {
	if ($screen_resolution > $resolutions[$i] OR $screen_resolution == $resolutions[$i]) {
		$breakpoint = $resolutions[$i];
		break;
	}
}
$req_img_size = $img_categories[$req_img_cat][$breakpoint];
$resolution = $req_img_size;
$requested_uri = str_replace($req_img_cat.'/', '', $requested_uri);
